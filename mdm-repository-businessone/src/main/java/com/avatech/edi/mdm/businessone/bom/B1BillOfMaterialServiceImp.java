...
public class B1BillOfMaterialServiceImp implements B1BillOfMaterialService {
    ...
    private final String LINE_REMARK = "U_Notes";

    private final String COSTCENTER4 = "CostCenter4";
    private final String COSTCENTER5 = "CostCenter5";
    ...
    public String approvedBillOfMaterial(IBillOfMaterial billOfMaterial, B1Connection b1Connection) {
        ...
        try{
            //过滤虚拟物料后冻结明细为空
            if(billOfMaterial.getCompontOfMaterialListItems().stream().filter(c->c.getIsLocked().equals("Y") && !c.getItemCode().equals(B1Data.VISUAL_ITEMCODE)).collect(Collectors.toList()).size() == 0)
                return "";
            boRepositoryBusinessOne = BORepositoryBusinessOne.getInstance(b1Connection);
            company = boRepositoryBusinessOne.getCompany();
    ...
    private String createOrUpdateProduceOrder(IBillOfMaterial billOfMaterial,ICompany company,Integer docEntry){
        try{
            ...
            if(billOfMaterial.getDeptType() != null && !billOfMaterial.getDeptType().isEmpty())
                document.getUserFields().getFields().item(DEPT_TYPE).setValue(billOfMaterial.getDeptType());
            List<Integer> deleteLineNum = new ArrayList<>();
            HashMap<String,String> costCenterMap = getCostCenter(billOfMaterial,company);
            if(costCenterMap != null && costCenterMap.containsKey(COSTCENTER4) && !costCenterMap.get(COSTCENTER4).isEmpty() ){
                document.setDistributionRule4(costCenterMap.get(COSTCENTER4));
            }
            if(costCenterMap != null && costCenterMap.containsKey(COSTCENTER5) && !costCenterMap.get(COSTCENTER5).isEmpty() ){
                document.setDistributionRule5(costCenterMap.get(COSTCENTER5));
            }

            for (ICompontOfMaterialListItem item: BillOfMaterial.createMaterialListItem(billOfMaterial)) {
                if(item.getIsLocked().equals("Y") && item.getQuantity() > 0 && !item.getItemCode().equals(B1Data.VISUAL_ITEMCODE)) {
                    ...
                    if(costCenterMap != null && costCenterMap.containsKey(COSTCENTER4) && !costCenterMap.get(COSTCENTER4).isEmpty() ){
                        document.getLines().setDistributionRule4(costCenterMap.get(COSTCENTER4));
                    }
                    if(costCenterMap != null && costCenterMap.containsKey(COSTCENTER5) && !costCenterMap.get(COSTCENTER5).isEmpty() ){
                        document.getLines().setDistributionRule5(costCenterMap.get(COSTCENTER5));
                    }
                    document.getLines().getUserFields().getFields().item(BASE_TYPE).setValue(OBJECT_CODE);
                    document.getLines().getUserFields().getFields().item(BASE_DOCENTRY).setValue(item.getDocEntry().toString());
                    document.getLines().getUserFields().getFields().item(BASE_LINENUM).setValue(item.getLineId().toString());
                    ...
                    if(item.getModelName() != null && !item.getModelName().isEmpty()){
                        document.getLines().getUserFields().getFields().item(LINE_REMARK).setValue(item.getRemark());
                    }
                    document.getLines().add();
                }else if(item.getIsLocked().equals("N") && item.getDesLineNum()>=0){
                    deleteLineNum.add(item.getDesLineNum());
                }
            }
            if(isExists){
                // 删除解冻的行
               for (int i=0;i<deleteLineNum.size();i++){
                   document.getLines().setCurrentLine(deleteLineNum.get(i)-i);
        ...
        try {
            ...
            for (ICompontOfMaterialListItem item:compontOfMaterialListItems) {
                if(item.getIsLocked().equals("Y") && item.getQuantity() > 0 && !item.getItemCode().equals(B1Data.VISUAL_ITEMCODE)){
                    ...
                    document.getLines().setProjectCode(billOfMaterial.getProject());
                    if(costCenterMap != null && costCenterMap.containsKey(COSTCENTER4) && !costCenterMap.get(COSTCENTER4).isEmpty() ){
                        document.getLines().setCostingCode4(costCenterMap.get(COSTCENTER4));
                    }
                    if(costCenterMap != null && costCenterMap.containsKey(COSTCENTER5) && !costCenterMap.get(COSTCENTER5).isEmpty() ){
                        document.getLines().setCostingCode5(costCenterMap.get(COSTCENTER5));
                    }
                    document.getLines().getUserFields().getFields().item(BOM_WORKORDERNUM).setValue(billOfMaterial.getWorkOrderNo());
                    SimpleDateFormat sdf = new SimpleDateFormat( " yyyy-MM-dd " );
                    document.getLines().getUserFields().getFields().item(DOCDATE).setValue(sdf.format(item.getDocDate()));
            ...
            for (int j=0;j<orders.size();j++){
                if(orders.get(j).getBaseLineNum().equals(billOfMaterialItems.get(i).getLineId())
                    ...
                    if(billOfMaterialItems.get(i).getQuantity() > orders.get(j).getQuantity()){
                        ...
                        billOfMaterialItems.get(i).setIsLocked("N");
                    }
                    break;
                }
            }
    ...
    private HashMap<String,String> getCostCenter(IBillOfMaterial billOfMaterial, ICompany company){
        try{
            IRecordset res = SBOCOMUtil.newRecordset(company);
            String sqlCostCenter = "select ifnull(\"OcrCode4\",'') as \"OcrCode4\",ifnull(\"OcrCode5\",'') as \"OcrCode5\"  from RDR1 t0\n" +
                    "inner join (select top 1 \"U_ItemCode\" from \"@AVA_PM_OVCO\" t1\n" +
                    "where t1.\"U_ItemType\" = 'C' \n" +
                    "and exists(select 1 from \"@AVA_PM_OVCO\" t2 where t2.\"DocEntry\" = %s and t1.\"U_Muid\" = t2.\"U_Muid\")) t3\n" +
                    "on t0.\"ItemCode\" = t3.\"U_ItemCode\" ";
            res.doQuery(String.format(sqlCostCenter,billOfMaterial.getDocEntry()));
            if(res.getRecordCount() > 0){
                HashMap<String,String> costCenterMap = new HashMap<>();
                if(res.getFields().item("OcrCode4").getValue() != null || !res.getFields().item("OcrCode4").getValue().toString().isEmpty()){
                    costCenterMap.put(COSTCENTER4,res.getFields().item("OcrCode4").getValue().toString());
                }
                if(res.getFields().item("OcrCode5").getValue() != null || !res.getFields().item("OcrCode5").getValue().toString().isEmpty()){
                    costCenterMap.put(COSTCENTER5,res.getFields().item("OcrCode5").getValue().toString());
                }
                return costCenterMap;
            }
            return null;
        }catch (Exception e){
            logger.error("查询成本中心信息异常",e);
            throw new B1Exception(e);
        }
    }
